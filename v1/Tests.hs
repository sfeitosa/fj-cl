...
instance Arbitrary Expr where
  arbitrary = do ct <- genClassTable
                 ...
                 sized (\n -> genExpr n True ct Data.Map.empty t)
...
prop_genwellformedct =
  forAll (genClassTable) $
    \ct -> Data.List.all 
             (\(nm, t) -> case (t) of 
                            (TClass cl) -> classTyping ct Data.Map.empty cl
                            (TInterface i) -> interfaceTyping ct Data.Map.empty i
             ) (Data.Map.toList ct)
...
prop_gencastsafeexpr = 
  forAll (genClassTable) $ 
    ...
    \e -> case e of 
            (Cast t' e) -> 
              case (typeof ct Data.Map.empty (lambdaMark e (Type t'))) of
...
prop_preservation =
  forAll (genClassTable) $
    ...
    \e -> either (const False)
                 ...
                 (case (eval' ct e) of 
                    Just e' -> typeof ct Data.Map.empty e'
