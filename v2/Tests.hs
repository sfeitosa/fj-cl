...
prop_genwellformedct =
  forAll (genClassTable) $
    \ct -> Data.List.all 
             (\(nm, t) -> 
               case (t) of 
                 (TClass cl) -> case (classTyping ct Data.Map.empty cl) of
                                  (r,_) -> r
                 (TInterface i) -> case (interfaceTyping ct Data.Map.empty i) of
                                     (r, _) -> r
             ) (Data.Map.toList ct)
...
prop_gencastsafeexpr = 
  forAll (genClassTable) $ 
    ...
    \e -> case e of 
            c@(Cast t' e') -> 
              case (typeof ct' Data.Map.empty c) of
